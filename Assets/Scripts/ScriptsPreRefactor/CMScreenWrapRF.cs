...
{	
	...
	void Update()
	{
        if (setOrtoSize)
        {
            if (advancedWrapping)
            {
                ...
                //Should called if renderes change by event or action but works fine by the moment
                //renderers = GetComponentsInChildren<Renderer>();
            }
            else
            {
	...
	void CreateGhostShips()
	{
		for(int i = 0; i < 2; i++)
		{
            ...
            //Destroy(ghosts[i].GetComponent<TouchMovementRF>());
            Destroy(ghosts[i].GetComponent<TouchMovement>());
            Destroy(ghosts[i].GetComponent<Rigidbody>());
            //Destroy(ghosts[i].GetComponentInChildren<SpriteRenderer>());
